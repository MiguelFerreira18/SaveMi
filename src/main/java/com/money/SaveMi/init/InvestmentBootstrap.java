...
public class InvestmentBootstrap implements CommandLineRunner {
    ...
    private void saveInvestment(User u, Currency c, StrategyType strategyType, int numberOfInvestments) {
        for(int i = 1; i <= numberOfInvestments; i++){
            ...
            if (investmentRepo.findByUserIdAndStrategyIdAndCurrencyIdAndAmount(u.getId(),strategyType.getId(),c.getId(),value).isEmpty()) {
                Investment expense = new Investment(u, c, value,  strategyType,description, LocalDate.now().plusDays(i));
